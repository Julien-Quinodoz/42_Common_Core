...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/11/11 17:21:42 by jquinodo          #+#    #+#             */
/*   Updated: 2025/12/12 01:13:53 by jquinodo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../inc/CgiHandler.hpp"
#include "../../network_layer/inc/Logger.hpp"
...
void CgiHandler::initEnvCgi(HttpRequest& req, const std::vector<Location>::iterator it_loc)
{
	...
	this->_env["SERVER_SOFTWARE"] = "LETSGO";
...
void CgiHandler::execute(short &error_code)
{
	...
	{
		// Parent: fermeture des pipes non utilisées
		close(pipe_in[0]);  //	Parent ne lit pas de pipe_in
		close(pipe_out[1]);  //	 Parent ne écrit pas dans pipe_out

		// Définition des pipes en mode non-bloquant pour éviter le blocage du serveur
		int flags_in = fcntl(pipe_in[1], F_GETFL, 0);
		int flags_out = fcntl(pipe_out[0], F_GETFL, 0);

		if (flags_in < 0 || fcntl(pipe_in[1], F_SETFL, flags_in | O_NONBLOCK) < 0)
		{
			Logger::error("Failed to set pipe_in[1] to non-blocking");
			close(pipe_in[1]);
			close(pipe_out[0]);
		...
		if (flags_out < 0 || fcntl(pipe_out[0], F_SETFL, flags_out | O_NONBLOCK) < 0)
		{
			Logger::error("Failed to set pipe_out[0] to non-blocking");
			close(pipe_in[1]);
			close(pipe_out[0]);
			waitpid(this->_cgi_pid, NULL, 0);
			error_code = 500;
...
/* Traduction des paramètres pour la variable d'environnement QUERY_STRING */
...
/* Isolation de la variable PATH_INFO */
